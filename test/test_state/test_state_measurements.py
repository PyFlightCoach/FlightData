...
def test_boundary_measure_rate_reduces_as_window_widens():
    ...
    rate, dmr1, dmr2 = st.boundary_measure(
        "measure_rate", 0.199, 0.401, st0[0].att, g.PY(), 0.01
    )
...
def test_measure_rate(state: State):
    _st = state[156.844:158.365]

    iatt = _st[0].att.closest_principal()
    t0 = _st.t[0]
    t1 = _st.t[-1]

    assert (
        state.boundary_measure("measure_rate", t0, t1 + 0.001, iatt)[0]
        < (state.boundary_measure("measure_rate", t0, t1, iatt))[0] ** 2
    )
    assert (state.boundary_measure("measure_rate", t0, t1 - 0.001, iatt)[0] ** 2) > (
        state.boundary_measure("measure_rate", t0, t1, iatt)[0] ** 2
    )
...
def test_measure_duration(state: State):
    ...
    assert state.boundary_measure("measure_duration", t0, t1, None, None)[0] == approx(
        t1 - t0, rel=1e-2
    )
